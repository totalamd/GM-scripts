...
(function(){
	if (!document.querySelector('a[href^="magnet:?"]')) {
		const link = document.querySelector('span#tor-hash');
		const hash = link.innerHTML;
		link.outerHTML = `<a href="magnet:?xt=urn:btih:${hash}&tr=http://bt.rutracker.cc/ann?magnet" class="med magnet-link-16" title="Скачать по magnet-ссылке"><img src="http://static.rutracker.org/templates/v1/images/icon_magnet_16_1.png" alt="magnet"><span id="tor-hash">${hash}</span></a>`;
	}
}())